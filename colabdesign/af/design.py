...
class _af_design:
    ...
    def _crossover(self, seq1, seq2, plddt1=None, plddt2=None, max_len=20, MIN_LEN=10):
      ...
      plddt1_chunks = np.split(plddt1, np.cumsum(chunks1))
      
      seq2_chunks = np.split(seq2, np.cumsum(chunks2))
      plddt2_chunks = np.split(plddt2, np.cumsum(chunks2))
      
      # pop the last empty chunk
      seq1_chunks.pop()
      plddt1_chunks.pop()
      seq2_chunks.pop()
      plddt2_chunks.pop()
      ...
      sequence = []
      # crossover where plddt is higher in seq2
      for i in range(len(seq1_chunks)):
        ...
        if plddt1_chunks[i].mean() > plddt2_chunks[0].mean():
    ...
    def design_mapelites(
        ...
        cycle_offset=False,
        experiment_name="mapelites_experiment",
        **kwargs,
    ):
        ...
        sequences = []
        if init_archive:
            print("Initializing archive with initial sequences")
